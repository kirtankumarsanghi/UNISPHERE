...
type Placement = { avgPackage: number; highestPackage: number; medianPackage: number; placementPercent: number; topRecruiters: string[]; year: number } | null;
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="rounded-2xl border border-border bg-surface p-4">
        ...
        <div className="mt-3 flex flex-wrap gap-2">{placement.topRecruiters.map((r) => <span key={r} className="rounded-lg border border-border bg-surface2 px-3 py-1.5 text-xs text-muted">{r}</span>)}</div>
        <p className="mt-3 text-xs text-muted">Placement data for batch {placement.year}</p>
      </div>
    </section>
  );
}